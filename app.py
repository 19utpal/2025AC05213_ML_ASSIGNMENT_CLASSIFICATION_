...
DATA_FILE_CANDIDATES = [
    APP_DIR / "test_csv.csv",
    APP_DIR / "streamlit_cloud_single_file" / "test_csv.csv",
]
...
def load_reference_data() -> pd.DataFrame:
    for data_file in DATA_FILE_CANDIDATES:
        if data_file.exists():
            return pd.read_csv(data_file)

    checked_paths = "\n".join(f"- {path}" for path in DATA_FILE_CANDIDATES)
    st.error(
        "Could not find `test_csv.csv`. Streamlit Cloud must deploy the CSV with the app. "
